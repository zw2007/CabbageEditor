...
## 常见问题（与修复）
- 删除 bridge.py 后无法创建 Dock：
  - 可能原因：AIService 仍从 bridge 导入 WorkerThread，致使服务注册失败 → 已将 WorkerThread 内联在 AIService 内，避免该问题。
- add_dock_widget 槽未命中：
  - 可能原因：前端传了对象但槽签名为 str → 现已使用 `@Slot(..., object)` 并在函数体兼容解析。
- Dock 内回流未命中主窗口：
  - 可能原因：CentralManager 未注册 creator/remover → 兜底会直接调用 MainWindow.browser_widget；建议仍在 BrowserWidget 初始化时注册。

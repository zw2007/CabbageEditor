...
└────┬────────┘
     │  调用
┌────▼────────────┐
│ Application 层  │  (Backend/application/services)
└────┬────────────┘
     │  依赖注入
┌────▼────────────┐
│ Infrastructure  │  (LLM/MCP、文件、配置、引擎包装)
└────┬────────────┘
     │
┌────▼────────────┐
│ Domain / Engine │  (SceneManager + CoronaEngine C++ 接口)
└─────────────────┘
```
...
| 目录 | 说明 |
|------|------|
| Backend/config/ | settings.py / secrets.py / defaults.toml |
...
2. **密钥**：执行 `python -m Backend.interfaces.cli.secrets`（或设置 `CORONA_API_KEY`、`CORONA_BASE_URL`）。
3. **后端**：`python Backend/main.py`。默认禁用 GPU，相关环境可在 `config/defaults.toml` 配置。
4. **MCP/AI**：`python Backend/mcp_client.py` 打开 CLI；`Backend/transform_server.py` 为 MCP 工具入口（内部引用 `interfaces.mcp.server`）。
...
- `LLMClient`：读取 settings/secrets，支持工具调用；当模型要求搜索时会自动插入 system 指令。
- `MCPToolAdapter`：在单进程内创建内存 stream，连接 `interfaces.mcp.server`；`list_tools` 自动兼容无 `input_schema` 的工具对象。
- `interfaces/mcp/server.py`：暴露 `transform_actor`、`list_actors` 两个工具，直接使用 `SceneApplicationService`。
...
| LLM 401 或工具异常 | 查看控制台 `AIService` 捕获的 JSON 错误；检查 `~/.coronaengine/credentials.toml` 与环境变量。 |
